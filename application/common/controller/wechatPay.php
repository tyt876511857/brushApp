...
$mchid = '1268392201';          //微信支付商户号 PartnerID 通过微信支付商户资料审核后邮件发送
$appid = 'wx04755290d7bab31f';  //微信支付申请对应的公众号的APPID

$appKey = '0cb976ef6d87de6bfc273a529c9aac65';   //微信支付申请对应的公众号的APP Key
$apiKey = 'e388c1c5df4933fa01f6da9f92595589';   //https://pay.weixin.qq.com 帐户设置-安全设置-API安全-API密钥-设置API密钥

//①、获取当前访问页面的用户openid（如果给指定用户转账，则直接填写指定用户的openid)
$wxPay = new WxpayService($mchid, $appid, $appKey, $apiKey);
//$openId = "oD_r8jipxmYCkMKre2c0krCVuAfI";
$openId = 'oD_r8jipxmYCkMKre2c0krCVuAfI';      //获取openid
...
class WxpayService
{
    ...
    public function createJsBizPackage($openid, $totalFee, $outTradeNo, $trueName)
    {
        ...
        $unified = array(
            ...
            'amount' => intval($totalFee * 1000 / 10),       //单位 转为分
            'desc' => '付款',            //企业付款操作说明信息
        );
        ...
        if ($unifiedOrder === false) {
            file_put_contents("log\wxpay.log", $openid . " " . $totalFee . " " . $responseXml . "\r\n", FILE_APPEND);

            return false;
        }
        if ($unifiedOrder->return_code != 'SUCCESS') {
            file_put_contents("log\wxpay.log", $openid . " " . $totalFee . " " . $unifiedOrder->return_msg . "\r\n", FILE_APPEND);

            return $unifiedOrder->return_msg;
        }
        if ($unifiedOrder->result_code != 'SUCCESS') {
            file_put_contents("log\wxpay.log", $openid . " " . $totalFee . " " . $unifiedOrder->err_msg . "\r\n", FILE_APPEND);
            return $unifiedOrder->return_msg;
        }
    ...
    public function curlPost($url = '', $postData = '', $options = array())
    {
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, getcwd() . '/cert1/apiclient_cert.pem');
        //默认格式为PEM，可以注释
        curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
        curl_setopt($ch, CURLOPT_SSLKEY, getcwd() . '/cert1/apiclient_key.pem');
