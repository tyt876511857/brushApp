                ...
                foreach ($taskList as $item) {
                    ...
                    if ($is_lock == true) { // 获取锁权限
                        ...
                        /*if (!empty($taskInfo['coupon_money'])) {
                            $item['principal'] = $taskInfo['goods_price'] - $taskInfo['coupon_money'];
                        } else {
                            $item['principal'] = $taskInfo['goods_price'];
                        }*/
                        $item['principal'] = $taskInfo['true_pay'];

                        //完成状态更新
                        //redis缓存维持
                        $redis->del($taskStateTimeKey.$item['uid']);
                        $redis->set($keyTaskEnd . $item['uid'],time(),$keyTaskEndTime);
                        ...
                        Db::name('goods_task')->where('id',$item['task_id'])->inc('finish_num')->update();
                        Db::name('goods_task_queue')->where('id',$item['task_queue_id'])->inc('finish_num')->update();
                        if ($redis->get($taskDoingNumKey.$item['task_id'])>0) {
                            $redis->decr($taskDoingNumKey.$item['task_id']);
                        }
                        if ($redis->get($childTaskDoingNumKey.$item['task_queue_id'])>0) {
                            $redis->decr($childTaskDoingNumKey.$item['task_queue_id']);
                        }
                        $verify = [
                            'uid'=>$item['uid'],
                            'deid'=>$item['id'],
                        ...
                        if ($orderInfoarr['orders']['order'][0]['refund_status'] != 'NO_REFUND' && $orderInfoarr['orders']['order'][0]['refund_status'] != 'CLOSED') {
                            ...
                            //任务改为审核失败
                            Db::name('deal_detail')->where('id',$item['id'])->update(['deal_state'=>9,'end_time'=>time()]);
                            //删除锁
                            $redis->del($lock_key);
                            continue;
                        }
